...
export async function LoadResources(...args) {
	...
	for(const resource of args) {
		...
		let type, path, important, shadow;
		if ( typeof resource === 'string' ) {
			([type, path, important, shadow] = ['js', resource, true, {}]);
		}
		else {
			({type, path, important, shadow={}} = resource);
		}
		...
		const R_PROMISE = new Promise((resolve, reject)=>{
			...
			switch( type[0] ) {
				...
				case "module": {
					ModuleImport(path).then(resolve).catch(reject);
					break;
				}
				case "js": {
					if ( type[1] !== "shadow" ) {
