	...
	get [CastHTTPStringSymbol]() {
		...
		if ( this.duration !== null ) {
			if ( typeof this.duration !== "number" || this.duration <= 0 ) {
				throw new TypeError( "Cookie duration must be a number greater than zero!" );
			}
...
export class HTTPCookies {
	...
	**/
	set(name, value, options={domain:null, path:'/', duration:0, sslOnly:false, httpOnly: true, sameSite:'strict'}) {
		const cookie = new HTTPCookieItem(name, value);
		cookie.domain = options.domain || null;
		cookie.path = options.path || null;
		cookie.duration = options.duration || 0;
		cookie.ssl_only = !!options.sslOnly;
		cookie.http_only = !!options.httpOnly;
		cookie.same_site_policy = options.sameSite || null;
